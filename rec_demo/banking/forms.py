...
class PaymentPlanForm(AuditableFormMixin, forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        transaction_type_qs = TransactionType.objects.filter(allowed_for=TransactionType.ALLOWED_FOR_DEBIT).all()
    ...
    def clean(self):
        cleaned_data = super(PaymentPlanForm, self).clean()
    ...
    def save(self, commit=True):
        instance = super(PaymentPlanForm, self).save(commit=commit)
    ...
    def _build_transaction_fields(self, i, transaction_type_qs, **kwargs):
        ...
        if transaction is not None:
            ...
            self.initial[field_name] = Transaction.DEBIT_TYPE
        field_names.append(field_name)
        # date
        # -----------------------------------------------------------
        field_name = self.TRANSACTION_PATTERN.format('date', i)
        self.fields[field_name] = forms.DateField(label=_('Date'), required=False, widget=forms.HiddenInput())
        if transaction is not None:
            self.initial[field_name] = transaction.date
        else:
            self.initial[field_name] = timezone.localtime().date()
        field_names.append(field_name)
        # amount
        # -----------------------------------------------------------
        ...
        field = forms.ChoiceField(label=_('Related debit'), required=False, widget=forms.HiddenInput())
